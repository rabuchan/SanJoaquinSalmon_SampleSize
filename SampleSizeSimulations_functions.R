...
# sim.data.f.v2         # simulate data using parameters named in par.names using values in par.values from multinomial likelihood with cell probabilities in like.vec and release size N.
# get.sA.3              # compute route-specific survival from Lathrop (site A1) to Chipps Island (site G2) via San Joaquin River route (= route A)
# get.sR.3              # compute total survival from release at Durham Ferry to Chipps Island (site G2)
...
# sim.supp.mle.pari.R1.R2    # Simulate data, calculate MLE of parameters using optim, return matrix of MLE estimates
...
sim.data.f.v2=function(N,par.values,par.names,var.vec,like.vec,nsim)
{
  # simulate data using parameters named in par.names using values in par.values from multinomial likelihood with
  # cell probabilities in like.vec and release size N.
...
sim.supp.seeds.pari.R1.R2=function(parvec,R1,R2,nSim,DF.pars,supp.pars,A1.pars,A2.pars,B1.pars,F1.pars,G2.pars,DF.vars,supp.vars,A1.vars,A2.vars,B1.vars,F1.vars,G2.vars,DF.like,supp.like,A1.like,A2.like,B1.like,F1.like,G2.like)
{
  ...
  mat.out[,1]=unlist(c(parvec[pars.supp.fit],get.sA.3(parvec),get.sR.3(parvec)))
  ...
  data.A1=sapply(unlist(lapply(stats.DF,f2.a,"A1")),sim.data.f.v2,par.values=parvec[A2.pars], par.names=A1.pars, var.vec=A1.vars, like.vec=A1.like, nsim=1)
  data.A2=sapply(unlist(lapply(stats.DF,f2.a,"A2"))+unlist(lapply(stats.supp,f2.a,"A2")),sim.data.f.v2,par.values=parvec[A2.pars], par.names=A2.pars, var.vec=A2.vars, like.vec=A2.like, nsim=1)
  data.B1=sapply(unlist(lapply(stats.DF,f2.a,"B1")),sim.data.f.v2,par.values=parvec[B1.pars], par.names=B1.pars, var.vec=B1.vars, like.vec=B1.like, nsim=1)
  data.F1=sapply(unlist(lapply(stats.DF,f2.a,"F1"))+unlist(lapply(stats.supp,f2.a,"F1")),sim.data.f.v2,par.values=parvec[F1.pars], par.names=F1.pars, var.vec=F1.vars, like.vec=F1.like, nsim=1)
  data.G2=sapply(unlist(lapply(stats.DF,f2.a,"G2"))+unlist(lapply(stats.supp,f2.a,"G2")),sim.data.f.v2,par.values=parvec[G2.pars], par.names=G2.pars, var.vec=G2.vars, like.vec=G2.like, nsim=1)
  ...
  seeds["sA",]=apply(seeds[1:18,],2,get.sA.3)
  seeds["sR",]=apply(seeds[1:18,],2,get.sR.3)
  mat.out[,-1]=seeds
  return(mat.out)
}
...
{
  # Simulate data, calculate MLE of parameters using optim, return matrix of MLE estimates
  ...
  for(i in 1:nSim)
  {
    ...
    j=1
    x.init=optim(seeds.tmp,log.like.f.v2.cmp,dat.vec=data.sim[,i], par.names=pars.supp.fit, var.vec=vars.all, like.vec=like.all, pars0=pars.0, pars1=pars.1,parsNA=pars.na) #, control=list(maxit=10000))
    ...
    {
      j=j+1
      x.step=optim(pars.old,log.like.f.v2.cmp,dat.vec=data.sim[,i],par.names=pars.supp.fit, var.vec=vars.all, like.vec=like.all, pars0=pars.0,pars1=pars.1,parsNA=pars.na) #,control=list(maxit=10000))
    ...
    sA=get.sA.3(pars.new)
    sR=get.sR.3(pars.new)
